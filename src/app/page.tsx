...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#050607] text-slate-200">
      <Navbar />
              ...
              {heroContacts.map((contact) => (
                <div key={contact.label} className="flex flex-col gap-1">
                  <div className="inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.3em] text-slate-500">
                    <contact.Icon className="h-4 w-4 text-slate-200" />
              ...
              >
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  {block.items.map((item) => (
                    ...
                    >
                      <span className="mt-1 flex h-6 w-6 items-center justify-center rounded-full border border-emerald-400/40 bg-emerald-400/15 text-xs font-semibold text-emerald-200">
                        +
                      </span>
                ...
                <ul className="space-y-3 text-slate-200">
                  {block.items.map((item) => (
                    <li key={item} className="flex items-start gap-3">
                      {isChecks ? (
                        <span className="mt-0.5 inline-flex h-5 w-5 items-center justify-center rounded-full bg-emerald-400/20">
        ...
        >
          <div className="flex flex-col gap-4 rounded-3xl border border-white/10 bg-gradient-to-r from-white/15 via-white/5 to-white/15 p-6 text-white shadow-[0_25px_80px_rgba(0,0,0,0.45)] md:flex-row md:items-center md:justify-between md:p-8">
            ...
            <div className="flex flex-wrap gap-3">
              <a
                href="mailto:harisibrahim1@yahoo.com?subject=Book%20a%20call&body=Hi%20Haris,%0D%0A%0D%0AI%20came%20across%20your%20portfolio%20and%20would%20love%20to%20discuss%20a%20potential%20engagement."
